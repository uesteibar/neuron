defmodule Neuron.ResponseTest do
  ...
  describe "when successful response" do
    ...
    test "returns the parsed Response struct", fixtures do
      result = Response.handle({:ok, fixtures.response})
      expected_result = %Response{
        body: %{"users" => []},
        headers: fixtures.response.headers,
        status_code: fixtures.response.status_code,
      }
  ...
  describe "when response with != 200 status code" do
    ...
    test "returns the parsed Response struct with errors", fixtures do
      result = Response.handle({:ok, fixtures.response})
      expected_result = %Response{
        body: %{
